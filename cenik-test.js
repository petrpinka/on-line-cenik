<script>
// CENÍK – verze 09-10-25, 16:28 (původní logika) + SAFE start & thead/tbody fallback
(function(){
  ...
  function start(){
    ...
    fetch(URL_JSON)
      .then(r => r.json())
      .then(data => {
        var table = document.getElementById("cenik-test"); // Ujistěte se, že sedí ID!
        if (!table) return;
        ...
        // ✅ FALLBACK: vytvoř thead/tbody, když chybí
        ...
        table.insertBefore(colgroup, thead); // ok i když thead právě vznikl

        // --- ZÁHLAVÍ ---
        thead.innerHTML = ""; // pro jistotu vyčistit
        ...
        // --- DATA ---
        tbody.innerHTML = ""; // pro jistotu vyčistit
        ...
        function clearHighlight(){
          ...
          overlayFoot.style.opacity = "0";
          if (lastCol !== -1) {
            if (thead.rows[0].cells[lastCol]) thead.rows[0].cells[lastCol].style
